...
def main():
    ...
    for j in range(len(bowling_2)):
        ...
        if all_out:
            break
        for k in range(1,7):
            if (LF_1.predict_proba([[inn,j,k,stadium,toss,on_field.peek(),bowling_2[j],Team_1,Team_2,Year]])[:,1])*r_f*r_f_o>0.297:
                r_f=0.8
                r_f_o-=0.1
                a=on_field.dequeue()
                total_runs_1+=a_r
    ...
    for j in range(len(bowling_1)):
        ...
        if all_out:
            break
        for k in range(1,7):
            if (LF_1.predict_proba([[inn,j,k,stadium,toss,on_field.peek(),bowling_1[j],Team_2,Team_1,Year]])[:,1])*r_f*r_f_o>0.297:
                r_f=0.8
                r_f_o-=0.1
                a=on_field.dequeue()
                total_runs+=a_r
